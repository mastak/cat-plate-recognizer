...
from .tools import get_mode_torch
...
class OptionsDetector(object):
    ...
    def create_model(self) -> NPOptionsNet:
        ...
        if self.model is None:
            self.model = NPOptionsNet(
                len(self.class_region), len(self.count_lines), self.height, self.width, self.batch_size
            )
            if mode_torch == "gpu":
                self.model = self.model.cuda()
        return self.model

    def prepare(self, base_dir: str, num_workers: int = 0, verbose: bool = True) -> None:
        """
        TODO: describe method
        """
        logger.debug("START PREPARING")
    ...
    def train(
        ...
        self.trainer = pl.Trainer(max_epochs=self.epochs, gpus=self.gpus, callbacks=[checkpoint_callback, lr_monitor])
        self.trainer.fit(self.model, self.dm)
        logger.debug("[INFO] best model path", checkpoint_callback.best_model_path)
        self.trainer.test()
        return self.model
    ...
    def save(self, path: str, verbose: bool = True) -> None:
        ...
        if self.model is not None:
            logger.debug("model save to {}".format(path))
