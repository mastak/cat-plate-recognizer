...
def save_in_yolo_format(
    image, target_boxes, path_to_res_ann, path_to_res_images, image_id, labels, debug=True, suffix=""
):
    ...
    for bbox, label_id in zip(target_boxes, labels):
        ...
        if debug or is_corrupted:
            cv2.rectangle(image, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 120, 255), 3)
...
def rotation_augumentation(
    image, target_boxes, path_to_res_ann, path_to_res_images, image_id, labels, angles=None, debug=False
):
    if angles is None:
        angles = [90, 180, 270]
    ...
    for image, target_boxes, angle in zip(variant_images, variants_bboxes, angles):
        save_in_yolo_format(
            image, target_boxes, path_to_res_ann, path_to_res_images, image_id, labels, suffix=f"_{angle}", debug=debug
        )
    ...
    for _id in tqdm.tqdm(image_list["_via_img_metadata"]):
        ...
        if is_generate_image_rotation_variants:
            rotation_augumentation(
                image, target_boxes, path_to_res_ann, path_to_res_images, image_id, labels, debug=debug
            )
        else:
            save_in_yolo_format(image, target_boxes, path_to_res_ann, path_to_res_images, image_id, labels, debug=debug)
