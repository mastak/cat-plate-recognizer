    ...
    def decode(self, t, length):
        ...
        if length.numel() == 1:
            ...
            for i in range(length.numel()):
                texts.append(self.decode(t[index : index + length[i]], torch.IntTensor([length[i]])))
                index += length[i]
        return texts
...
def decode_batch(net_out_value: torch.Tensor, label_converter: StrLabelConverter) -> str or List:
    texts = []
...
def print_prediction(model, dataset, device, label_converter, w=200, h=50, count_zones=16):
    import matplotlib.pyplot as plt
