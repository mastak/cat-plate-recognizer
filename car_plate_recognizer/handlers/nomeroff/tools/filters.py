...
def draw_box(
    image: np.ndarray, boxs: List[np.ndarray], color: Tuple[int] = (255, 0, 0), thickness: int = 2
) -> np.ndarray:
    for box in boxs:
...
def color_splash(
    image: np.ndarray, masks: List[np.ndarray], color: Union = (0, 255, 0), white_balance: int = 200
) -> List[np.ndarray]:
    res = []
