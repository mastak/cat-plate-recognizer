...
from car_plate_recognizer.handlers.nomeroff.data_modules.data_loaders import normalize, aug_seed
from car_plate_recognizer.handlers.nomeroff.data_modules.numberplate_ocr_data_module import OcrNetDataModule
from car_plate_recognizer.handlers.nomeroff.nnmodels.ocr_model import NPOcrNet, weights_init
from car_plate_recognizer.handlers.nomeroff.tools import get_mode_torch
from car_plate_recognizer.handlers.nomeroff.tools.ocr_tools import StrLabelConverter, decode_prediction, decode_batch

mode_torch = get_mode_torch()


class OCR:
    LATEST_MODEL_OCR = "resources/TextDetector/Eu/anpr_ocr_eu_2021_09_27_pytorch_lightning.ckpt"
    ...
    def explain_text_generator(self, train_dir: str, letters: list, max_plate_length: int) -> None:
        pass

    def prepare(self, path_to_dataset: str, use_aug: bool = False, verbose: bool = True, num_workers: int = 0) -> None:
        ...
        if verbose:
            print("GET ALPHABET")
        self.letters, self.max_plate_length = self.get_alphabet(train_dir, test_dir, val_dir, verbose=verbose)
        self.init_label_converter()

        if verbose:
            print("\nEXPLAIN DATA TRANSFORMATIONS")
            self.explain_text_generator(train_dir, self.letters, self.max_plate_length)
    ...
    def train(
            self, log_dir=os.path.abspath(os.path.join(os.path.dirname(__file__), "../data/logs/ocr")), seed: int = None
    ) -> NPOcrNet:
        """
        TODO: describe method
        """
        ...
        self.trainer = pl.Trainer(
            max_epochs=self.epochs, gpus=self.gpus, callbacks=[checkpoint_callback, lr_monitor], weights_summary=None
        )
    ...
    def predict(self, imgs: List, return_acc: bool = False) -> Any:
        xs = []
        for img in imgs:
            x = normalize(img, width=self.width, height=self.height)
            xs.append(x)
        pred_texts = []
        net_out_value = []
        if bool(xs):
        ...
        if return_acc:
            if len(net_out_value):
                ...
                net_out_value = net_out_value.reshape(
                    (net_out_value.shape[1], net_out_value.shape[0], net_out_value.shape[2])
                )
    ...
    def load_model(self, path_to_model):
        if mode_torch == "gpu":
            self.model = NPOcrNet.load_from_checkpoint(
                path_to_model,
    ...
    def load(self, path_to_model: str = "latest") -> NPOcrNet:
        ...
        if path_to_model == "latest":
            path_to_model = self.LATEST_MODEL_OCR
    ...
    def get_acc(self, predicted: List, decode: List) -> torch.Tensor:
        ...
        acc = functional.ctc_loss(logits, encoded_texts, logits_lens.to(device), text_lens)
