...
from ..tools.image_processing import rotate_image_and_bboxes
from ..tools.ocr_tools import is_valid_str
...
def normalize(
        img: np.ndarray, height: int = 64, width: int = 295, to_gray: bool = False, with_aug: bool = False
) -> np.ndarray:
    if with_aug:
...
class ImgGenerator(torch.utils.data.Dataset):
    ...
    def get_x_from_path(self, path: str) -> np.ndarray:
        img = cv2.imread(path)
        x = normalize(img, with_aug=self.with_aug, width=self.img_w, height=self.img_h)
    ...
    def build_data(self) -> None:
        ...
        for i, (img_filepath, disc) in enumerate(self.samples):
            ...
            self.discs.append([np.eye(self.labels_counts[0])[disc[0]], np.eye(self.labels_counts[1])[disc[1]]])
