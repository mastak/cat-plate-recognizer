...
def split_boxes(
    bboxes: List[Union[np.ndarray, np.ndarray]], dimensions: List[Dict], similarity_range: int = 0.5
...
def detectDistanceFromPointToLine(matrix: List[np.ndarray], point: Union) -> float:
    """
...
def addoptRectToBbox(
    targetPoints: List,
    Bbox: Tuple,
    distansesoffsetLeftMaxPercentage: float,
    offsetTopMaxPercentage: float,
    offsetRightMaxPercentage: float,
    offsetBottomMaxPercentage: float,
) -> np.ndarray:
    """
    TODO: describe function
    """
...
class NpPointsCraft(object):
    ...
    def __init__(self, low_text=0.4, link_threshold=0.7, text_threshold=0.6, canvas_size=1280, mag_ratio=1.5):  # 0.4
    ...
    def loadModel(
        ...
        device: str = "cuda",
        is_refine: bool = True,
        trained_model: str = LATEST_MODEL_NP_POINTS_CRAFT_MLT,
        refiner_model: str = LATEST_MODEL_NP_POINTS_CRAFT_REFINDER,
    ) -> None:
        """
        TODO: describe method
        """
    ...
    def detectByImagePath(
        self, image_path: str, target_boxes: List[Dict], qualityProfile: List = None
    ) -> Tuple[List[Dict], Any]:
        """
        TODO: describe method
        """
    ...
    def detect_mline(self, image: np.ndarray, targetBoxes: List, qualityProfile: List = None) -> Tuple:
        ...
        for targetBox in targetBoxes:
            ...
            if len(propably_points):
                ...
                all_points.append([[x, y + h], [x, y], [x + w, y], [x + w, y + h]])
        return all_points, all_mline_boxes

    def detectInBbox(self, image: np.ndarray, craft_params: Dict = None, debug: bool = False):
        """
        TODO: describe method
        """
